...
class _NewsFeedState extends State<NewsFeed> {
      ...
      body: Center(
        child: Container(
          ...
          child: ListView.separated(
            itemCount:
                _feedItems.length + 1, // Add 1 for the post publishing section
            separatorBuilder: (BuildContext context, int index) {
              return const Divider();
            },
            itemBuilder: (BuildContext context, int index) {
              if (index == 0) {
                return _buildPublishPostSection();
              }
              final item = _feedItems[index - 1]; // Adjust index for feed items
              return Padding(
  ...
  Widget _buildPublishPostSection() {
    return Padding(
      padding: const EdgeInsets.all(8.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  void _publishPost() {
    ...
    // For demonstration, let's just add the new post to the beginning of the feed
    setState(() {
      _feedItems.insert(
        0,
        FeedItem(
          ...
          imageUrl: imageUrl,
          user: _users[
              0], // Assuming the current user is the first user in _users list
          likesCount: 0,
          commentsCount: 0,
          retweetsCount: 0,
        ),
    ...
    // Clear the text fields after publishing the post
